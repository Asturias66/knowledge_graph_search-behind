...
class QuestionClassifier:
    def __init__(self):
        ...
        #　特征词路径
        self.disease_path = cur_dir+'/dict/disease.txt'
        # print("disease_path:", self.disease_path)
        self.department_path =cur_dir+'/dict/department.txt'
        self.check_path =cur_dir+'/dict/check.txt'
        self.drug_path =cur_dir+'/dict/drug.txt'
        self.food_path =cur_dir+'/dict/food.txt'
        self.producer_path =cur_dir+'/dict/producer.txt'
        self.symptom_path =cur_dir+'/dict/symptom.txt'
        self.deny_path =cur_dir+'/dict/deny.txt'

        # 加载特征词
        self.disease_wds= [i.strip() for i in open(self.disease_path,'r',encoding='utf-8') if i.strip()]
        # print("self.disease_wds=:",self.disease_wds)
        ...
        # print("self.region_tree:",self.region_tree)
        # 构建词典
        self.wdtype_dict = self.build_wdtype_dict()
        # print("self.wdtype_dict:", self.wdtype_dict)
        # 问句疑问词
    ...
    def classify(self, question):
        print("分类主函数")
        data = {}
        medical_dict = self.check_medical(question)     #调用check_medical函数，获取问句中包含的领域词及其所在领域,并收集问句当中所涉及到的实体类型
        print("分类主函数下medical_dict:",medical_dict)
        if not medical_dict:
            return {}
        ...
        question_types = []
        print("self.symptom_qwds:",self.symptom_qwds)
        print("question:", question)
        print("types:",types)

        # 调用check_word函数，看问句中是否包含某领域特征词以及该领域是否在问句中包含的region_words的实体类型（types）里，以此来判断问句属于哪种类型。
        # 症状
        ...
        if self.check_words(self.cureway_qwds, question) and 'symptom' in types:
            question_type = 'disease_cureway'
            question_types.append(question_type)
    ...
    '''问句过滤，通过ahocorasick库的iter()函数匹配领域词，将有重复字符串的领域词去除短的，取最长的领域词返回。
    功能为过滤问句中含有的领域词，返回{问句中的领域词：词所对应的实体类型}。'''
    def check_medical(self, question):
        print("问句过滤")

        region_wds = []
        # ahocorasick库匹配问题，iter返回一个元组，i的形式如(3, (23192, '乙肝'))
